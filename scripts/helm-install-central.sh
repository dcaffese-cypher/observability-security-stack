...
CHART_DIR="$(cd "$SCRIPT_DIR/../charts/observability-central" && pwd)"
...
case "$MODE" in
  ...
  production|prod)
    if [[ ! -f values.local.yaml ]]; then
      ...
      echo "Replace YOUR_DOMAIN / YOUR_STORAGE_CLASS / YOUR_GITHUB_ORG, then re-run."
      exit 1
    fi
...
if [[ "$MODE" == "lab" ]]; then
  ...
  echo "  Open http://localhost:3000  (user: admin, password from create-grafana-secret.sh)"
else
